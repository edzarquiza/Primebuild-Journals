...
def process_raw_file(file_bytes, filename, payment_date_str):
    ...
    if freq == 'WC':
        internal_ref = f"{state} WComp PAY {fmt_date}"
    else:
        internal_ref = f"{state} {freq} PAY {fmt_date}"
    ...
    for _, row in data.iterrows():
        ...
        # Col H: GL Account default
        col_h = f"{default_cwi}{acct_no}"
...
def build_output_workbook(rows, state, freq, cwi, payment_date_str):
    ...
    title_font = Font(name='Arial', bold=True, size=10)
    ...
    # ── Row 3: Batch data ──
    ws.cell(row=3, column=2, value=cwi)
    ws.cell(row=3, column=3, value=dt).number_format = 'DD/MM/YYYY'
    ws.cell(row=3, column=5, value='G')
    ...
    for row_data in rows:
        ...
        for col_i, key in enumerate(OUTPUT_HEADERS, start=1):
            ...
            cell.font = data_font
            # GL Account and Costing Work Id as text
